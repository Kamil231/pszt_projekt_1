...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 26 23:58:44 2019
...
from DiffEv_Functions import init_population, apply_fitness
from DiffEv_Functions import current_to_pbest_mutation, crossover, selection, keep_bounds
from DiffEv_Functions import current_to_pbest_weighted_mutation


def get_default_params_jso(dim: int):
    """
...
def apply_jso(population_size: int, individual_size: int, bounds: np.ndarray,
    ...
    while num_evals < max_evals:
        ...
        mutated = current_to_pbest_weighted_mutation(population, fitness, f.reshape(len(f), 1),
                                                                   weighted, p, bounds)
        crossed = crossover(population, mutated, cr.reshape(len(f), 1))
        c_fitness = apply_fitness(crossed, func, opts)

        num_evals += current_size
        population, indexes = selection(population, crossed,
